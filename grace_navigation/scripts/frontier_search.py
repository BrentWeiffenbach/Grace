...
class FrontierSearch:
    ...
    def compute_centroids(self) -> Tuple[List[Point], List[float]]:
        ...
        obstacle_mask = cv2.inRange(
            image,
            np.array([255], dtype=image.dtype),
            np.array([255], dtype=image.dtype),
        )

        # Dilate obstacle mask
        obstacle_dilated = cv2.dilate(
            obstacle_mask, cv2.getStructuringElement(cv2.MORPH_RECT, (6, 6))
        )
        ...
        for cnt in contours:
            for point in cnt:
                ...
                if dilated_image[y, x] == 128:  # Gray pixel
                    ...
                    if any(
                        0 <= nx < dilated_image.shape[1]
                        and 0 <= ny < dilated_image.shape[0]
                        and dilated_image[ny, nx] < 50
        ...
        obstacle_frontier_mask = cv2.bitwise_and(
            frontier_mask, inverted_obstacle_dilated
        )

        # Threshold the image to black and white
        _, inflation_layer = cv2.threshold(
            self.global_costmap_img, 127, 255, cv2.THRESH_BINARY
        )

        # Apply connected components analysis
        _, labels, stats, _ = cv2.connectedComponentsWithStats(
            inflation_layer, connectivity=8
        )

        # Find the largest white blob (excluding background)
        largest_label = 1 + np.argmax(
            stats[1:, cv2.CC_STAT_AREA]
        )  # Skip the first background component
        ...
        clean_inflation_mask = cv2.resize(
            clean_inflation_mask,
            (obstacle_frontier_mask.shape[1], obstacle_frontier_mask.shape[0]),
        ...
        valid_frontier_mask = cv2.bitwise_and(
            obstacle_frontier_mask, clean_inflation_mask
        )
        ...
        _, labels, stats, centroid_cv = cv2.connectedComponentsWithStats(
            valid_frontier_mask, connectivity=4
        )

        # Sort components by area in descending order, excluding the background (label 0)
        sorted_indices = (
            np.argsort(stats[1:, cv2.CC_STAT_AREA])[::-1] + 1
        )  # Add 1 to skip background
        ...
        for idx in top_indices:
            if idx < len(centroid_cv):
                ...
                map_coords = self.convert_img_coords_to_map_coords(
                ...
                )  # Use the area of the component as its size
        ...
        if save_imgs:
            ...
            def imwrite(filename: str, image: cv2.typing.MatLike) -> bool:
            ...
            imwrite("1 map.png", self.map_img)
            imwrite("2 explored.png", binary_gray)
            imwrite("3 edges.png", edges)
            imwrite("4 obstacle_dialated.png", obstacle_dilated)
            imwrite("5 frontier_mask.png", frontier_mask)
            imwrite("6 obstacle_frontier_mask.png", obstacle_frontier_mask)
            imwrite("7 valid_frontier_mask.png", valid_frontier_mask)
            # cv2.imwrite("8 centroids.png", output)

            imwrite("9 inflation_layer.png", inflation_layer)
            imwrite("10 clean_inflation_mask.png", clean_inflation_mask)
            imwrite("11 global_costmap.png", self.global_costmap_img)
        return centroids, sizes

    def is_pose_in_occupancy_grid(
        self, pose: Pose, use_inflation_layer: bool = False
    ) -> bool:
        """
        Args:
    ...
    def is_point_in_occupancy_grid(
        self, point: Point, use_inflation_layer: bool = False
    ) -> bool:
        """
        Args:
