...
class TransformNode:
    def __init__(self):
        ...
        self.transformed_point_pub = rospy.Publisher(
            name="/transform_node/base_link_frame_point", data_class=PointStamped, queue_size=5
        )

        # Subscribers
        # scripts/yolo_detect.py
        self.camera_point_sub = rospy.Subscriber(
            name="/yolo_detect/detections/objects", data_class=PointStamped, callback=self.transform_point
        )
